...
def calc(n, theta, length, k, sigma, f, F):
    # W = ((length ** 2) / (2 * n)) * np.exp(-1 * ((k * length * np.sin(
    # theta)) ** 2) / n)
    # W = ((np.pi / n) ** 0.5) * length * np.exp(-1 * ((k * length * np.sin(
    # theta)) ** 2) / n)
    W = ((length / n) ** 2) * ((1 + (
                (4 * (k ** 2) * (np.sin(theta) ** 2) * (length ** 2)) / (
                    n ** 2))) ** (-1.5))
...
def surface(theta, length, lambda_wave, sigma, f_hh, f_vv, F_hh, F_vv,
            cutoff=1e-16):
    k = (2 * np.pi) / lambda_wave
    i = 1
    indicator = 1 / np.math.factorial(i)
    invariant = ((k ** 2) / 2) * np.exp(
    ...
    sigma_surf_hh = 10 * np.log10(sigma_hh)
    sigma_surf_vv = 10 * np.log10(sigma_hh)

    return sigma_hh, sigma_vv, sigma_surf_hh, sigma_surf_vv
...
def transmission(theta, eps):
    ...
    T_hh = ((2 * np.cos(theta) * np.sin(theta_t)) / (
        np.sin(theta + theta_t))) ** 2
